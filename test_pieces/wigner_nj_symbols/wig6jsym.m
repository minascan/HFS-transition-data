...
function w6j = wig6jsym(j1,j2,j3,l1,l2,l3)
...
%
% Updated to produce symbolic expression, august 2007
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if j1 >= 0 & j2 >= 0 & j3 >= 0 & l1 >= 0 & l2 >= 0 & l3 >= 0 & ... % quantities >= 0
   ...
   a = j1; b = j2; c = j3;
   d1 = sym(sqrt(factorial(a+b-c)*factorial(a-b+c)*factorial(-a+b+c)/factorial(a+b+c+1)));
   a = j1; b = l2; c = l3;
   d2 = sym(sqrt(factorial(a+b-c)*factorial(a-b+c)*factorial(-a+b+c)/factorial(a+b+c+1)));
   a = l1; b = j2; c = l3;
   d3 = sym(sqrt(factorial(a+b-c)*factorial(a-b+c)*factorial(-a+b+c)/factorial(a+b+c+1)));
   a = l1; b = l2; c = j3;
   d4 = sym(sqrt(factorial(a+b-c)*factorial(a-b+c)*factorial(-a+b+c)/factorial(a+b+c+1)));
   ...
   for k = kmin:kmax
     ...
     w6j = w6j + (-1)^k*sym(numerator)/sym(denominator);
   end
   w6j = w6j*d1*d2*d3*d4; 
else
   w6j = 0;
